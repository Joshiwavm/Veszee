...
class MsReader:
    ...
    def uvdata_loader(self):
        ...
        for spw in self.spws:
            for field in self.fields:
                ...
                uvwghts = np.append(uvwghts, uvwght*1e-3)
    ...
    def model_to_ms(self, model, scale, todo, savename):
        ...
        for spw in self.spws:
            for field in self.fields:
                ...
                uvreal = np.swapaxes(uvreal, 0,1)
                uvimag = np.swapaxes(uvimag, 0,1)
                
                if   (todo== 'replace'): rec['data'] = scale*np.array([(uvreal+1.0j*uvimag),(uvreal+1.0j*uvimag)])
                elif (todo=='subtract'): rec['data'] = np.copy(rec['data']) - scale*np.array([(uvreal+1.0j*uvimag),(uvreal+1.0j*uvimag)]) 
        ...
        if  (todo== 'replace'): os.rename(self.ms_modelfile, self.ms_copydir + 'Model_'+ savename + '.ms')
        elif (todo=='subtract'): os.rename(self.ms_modelfile, self.ms_copydir + 'Model-residue_'+ savename + '.ms')